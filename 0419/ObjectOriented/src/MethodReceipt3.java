...
public class MethodReceipt3 {
	...
	public static void main(String[] args) {
		int itemCount = 5;	//출력되는 항목 숫자 조절.
		titlePrint();	// 타이틀 출력 부분
		headerPrint(itemCount);	// 헤더(상품명, 단가, 수량, 금액) 인쇄함.
		
		for(int k41_i = 0; k41_i < itemCount; k41_i++) {	//항못 갯수만큼 인쇄
			itemPrint(k41_i);
		}
	...
	public static void headerPrint(int itemCount) {
		// 품명
		...
		for (int k41_i = 0; k41_i < itemCount; k41_i++) { // 품목 수만큼 반복하기.
	...
	public static void totalPrint(int k41_tax, int itemCount) {
		...
		System.out.printf("%17s/00119861/00164980/31",timeStamp(4));
	}
	public static void itemPrint(int k41_itemCount) {
		String[] k41_itemName = { "LG냉장고", "바나나우유", "건포도", "오렌지주스", "초코에몽", "자유시간", "세척당근(1kg/봉)", "냉동닭다리", "노브랜드도시락김",
				"월남쌈소스230g", "콘플레이크 500g", "양파(1.8kg/망)", "노브랜드믹스넛800g", "파프리카(혼합)", "강원 애호박", "부드러운치즈252",
				"햇브로콜리(2입/봉)", "후레쉬센터 흙대파", "남양 맛있는우유 1L", "자일리톨레인보우껌", "모란봉소불고기", "데톨핸드워시250ml(민감성피부용)", "노브랜드 핫베이컨칩",
				"펩시콜라(500ml*12/1Box)", "사조고추참치100g*3", "동원야채참치100g*3", "재사용봉투20L", "해태 후렌치파이초코맛", "롯데크런키3번들102g",
				"표고버섯(팩)" };
		System.out.printf("%s\n",k41_itemName[k41_itemCount]);
