...
public class SelectTables2 {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException {
		...
		String QueryTxt;
		for(int i = 0; i < 1000; i=i+30) { // 999까지 30씩 증가하는 i를 변수로 가지는 반복문 작성
			// examtable 테이블의 i번째부터 30개의 값을 전부 받고, 합, 평균의 값을 받아옵니다.
			QueryTxt = String.format(" select *, kor+eng+mat, (kor+eng+mat)/3 from examtable limit %d, 30;", i);
			// 실행한 명령어의 결과를 받아온다.
		    ...
		    QueryTxt = String.format(" select sum(kor), sum(eng), sum(mat), sum(kor+eng+mat), sum((kor+eng+mat)/3), "
		    		+ "avg(kor), avg(eng), avg(mat), avg(kor+eng+mat), avg((kor+eng+mat)/3) from (select kor,eng,mat from examtable limit %d,30)as sum;", i);
		    ...
		    QueryTxt = String.format(" select sum(kor), sum(eng), sum(mat), sum(kor+eng+mat), sum((kor+eng+mat)/3), "
		    		+ "avg(kor), avg(eng), avg(mat), avg(kor+eng+mat), avg((kor+eng+mat)/3) from (select kor,eng,mat from examtable limit 0, %d)as avg;", (i+30));
		    // 명령어 실행한 결과를 받아온다.
